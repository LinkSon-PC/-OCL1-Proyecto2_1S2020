%{
  /* cada Import es un NODO del arbol ATS
   el patron interprete dice que se tiene que crear todo modular una clase por nodo */
    ...
    const {Import} = require('../Otros/Import');
    const {ClaseInstruccion} = require('../Otros/ClaseInstruccion');

    const {Inicio} = require('../Otros/Inicio');
    const {Return} = require('../Instrucciones/Return');
    const {R_Funcion} = require('../Instrucciones/R_Funcion');

    const {Sentencia_Print} = require('../Instrucciones/Sentencia_Print');
    ...
    const {Declaracion_Parametros} = require('../Otros/Declaracion_Parametros');
...
decimal [0-9]+("."[0-9]+)
caracter (\'[^☼]\')
cadena (\"[^☼]*[\\"]*\")                  // FALTA ARREGLAR EL CASO DE COMILLAS ADENTRO DE COMILLAS 
...
{cadena}       {console.log("string LITERAL....");return 'STRING_LITERAL'}
...
.           CErrores.Errores.add(new CNodoError.NodoError("Lexico","No se esperaba el caracter: "+yytext,yylloc.first_line))
...
INICIO : LISTAS_ARCHIVOS EOF {$$ = new Tree($1);console.log("se genera el arbol"); return $$;}
       |  LISTA_IMPORTS EOF {$$ = new Tree($1);console.log("se genera el arbol"); return $$;}
       |  LISTA_CLASES  EOF{$$ = new Tree($1);console.log("se genera el arbol"); return $$;}
       | EOF{$$ = new Tree($1);console.log("se genera el arbol"); return $$;}
       ;

LISTAS_ARCHIVOS:  LISTA_IMPORTS LISTA_CLASES {let init =  new Inicio($1, $2); $$ = init.LISTAS_ARCHIVOS}
                     ;

LISTA_IMPORTS: LISTA_IMPORTS Import { $1.push($2); $$ = $1; }
      	 | Import    { $$ = [$1]; }
        ; 
...
Import: 'import' 'id' ';'   {$$ = new Import($2, $2 ,  this._$.first_line, this._$.first_column);}
       ;
...
AMBITO: 'void' OPCION_ID_MAIN '(' OPCION_METODO_FUNCION   { $$ = new DeclaracionMetodo($1, $2 , $4 ,  this._$.first_line , this._$.first_column);console.log("METODO");}
 ...
 OPCION_METODO_FUNCION: LISTA_PARAMETROS ')' BLOQUE_INSTRUCCIONES  {$$ = new Opcion_metodo_funcion($1 , $3 , this._$.first_line); console.log("CON PARAMETROS");}                                 
...
SENTENCIAIMPRIME: 'System' '.' 'out' '.'  OPCIONIMPRIME '(' EXPRESION ')' ';' { $$ = new Sentencia_Print($5,$7, this._$.first_line, this._$.first_column);}
                ;
...
INSTRUCCIONESWITCH : INSTRUCCIONESWITCH INSTRUCCIONSWITCH { $1.push($2); $$ = $1; }
              | INSTRUCCIONSWITCH               { $$ = [$1]; }
              |  error { console.error('Este es un error sintáctico: ' + yytext + ', en la linea: ' + this.$.first_line + ', en la columna: ' + this.$.first_column); CErrores.Errores.add(new CNodoError.NodoError("Sintactico","El error : "+yytext+" Columna:"+ this._$.first_column ,this._$.first_line)); }
              ;
...
Declaracion_Parametros: TIPO LISTA_IDS ASIGNACION { $$ = new Declaracion_Parametros($1,$2,$3 ,this._$.first_line , this._$.first_column ); console.log("dec adentro de metodos");}
                                        ;

/*
Declaracion_ParametrosP: LISTA_IDS ASIGNACION {$$ = new Declaracion_ParametrosP($1,$2);}
                                         ;     
*/   
...
LISTA_PARAMETROS :LISTA_PARAMETROS  ','  TIPO 'id'     { $1.push(new Parametro($3 , $4 ,this._$.first_line , this._$.first_column)); $$ = $1; }
...
SENTENCIA_CONTINUE: 'continue' ';' {$$ = new Continue( $1, this._$.first_line, this._$.first_column) ;}
                  ;
SENTENCIA_Return: 'return' ';' {$$ = new Return($1, this._$.first_line , this._$.first_column);}
                        ;
SENTENCIA_R_Funcion: 'return' EXPRESION ';' {$$ = new R_Funcion($1, $2 , this._$.first_line , this._$.first_column);}
                         ;
